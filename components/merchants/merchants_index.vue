<template>
    <div>
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                    <el-button type="success" icon="el-icon-edit" circle
                               @click="editmerchants(scope.row.mid)"></el-button>
        ...
        <el-dialog title="添加商户" :visible.sync="dialogVisible" width="50%">
        ...
        <el-dialog title="修改商户" :visible.sync="editDialogVisible" width="50%">
    ...
    export default {
        data() {
            ...
            delmerchants(val) { //删除数据
                ...
                _this.$axios.post("/deleteMerchants.action", params).then(function (result) {
                    _this.$message({
            ...
            editmerchants(val) { //编辑按钮按下  打开编辑模态框
                ...
                this.$axios.post("/queryMerchantsBYId.action", params)
                    .then(function (result) {
                        ...
                        _this.$refs.editmerchant.provincecode = str[0];
                        _this.method_sheng(str[0])
                        _this.method_shi(str[1])
                        _this.method_qu(str[2])
                        _this.$refs.editmerchant.merchants.mddress = str[3];
                    })
                    .catch(function (error) {
            ...
            method_sheng(sheng) {
                ...
                this.$axios.post("/queryProvinceName.action", params)
                    .then(function (result) {
                        _this.$refs.editmerchant.getcity2(result.data.code);
                        _this.$refs.editmerchant.provincecode= result.data.code;
                    })
                    .catch(function (error) {
                        //异步如果出现错误  触发catch里面的函数
                        alert(error);
                ...
                this.$axios.post("/queryCityName.action", params)
                    .then(function (result) {
                        _this.$refs.editmerchant.getarea2(result.data.code);
                        _this.$refs.editmerchant.citycode = result.data.code;
                    })
                    .catch(function (error) {
                        //异步如果出现错误  触发catch里面的函数
                        alert(error);
                ...
                this.$axios.post("/queryAreaName.action", params)
                    .then(function (result) {
                        _this.$refs.editmerchant.areacode = result.data.code;
                    })
                    .catch(function (error) {
                        //异步如果出现错误  触发catch里面的函数
                        alert(error);
